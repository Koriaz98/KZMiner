...
__global__ void testFillBlockShflKernel(uint64_t* memory, uint64_t* outResult);
...
int main()
{
    ...
    testFillBlockShflKernel<<<1, 32>>>(d_memory, d_out);
    ...
    printf("fillBlock FUSIONNE (shfl) obtenu (16 premiers octets) : ");
    for(int i = 0; i < 16; i++) printf("%02x", h_out[i]);
    printf("\n");
    printf("Reference sequentielle attendue (16 premiers octets)  : 0baea8f1a1f3fefa3a03cb57ea205b9e\n");

    std::vector<uint8_t> expected = readFile("/tmp/fillblock_expected.bin");
    if(expected.size() == 1024)
    {
        bool match = (memcmp(h_out, expected.data(), 1024) == 0);
        printf("%s\n", match ? "MATCH COMPLET (1024 octets)" : "MISMATCH sur les 1024 octets");
    }
    else
    {
        printf("(reference complete absente, comparaison 16 octets seulement)\n");
    }
